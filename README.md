...
**Ask in plain words.** The ✨ button beside the highlight box turns *"big videos I haven't opened in a year"* into `size>1gb ext:mp4,… used>1y` — and the translation is **always shown in an editable field before it runs**, because a natural-language box that hides what it actually searched is a black box, and this one is a query builder with a friendlier front door. The built-in phrase table works entirely offline; unknown words are listed as ignored, never silently dropped. Optionally, if you run **Ollama** yourself, TreeMap can ask it about phrasings the table doesn't know — strictly opt-in, off by default, your own endpoint only, the model's answer accepted only if the real parser accepts it, and with it off **zero network code runs** (a test proves that with a recorder server, not a promise).
