...
Committing runs a **dry run first, always**: the exact manifest, every path with its size, and anything that will be **left in place** with the reason. Then the whole batch goes through the **Time Capsule** as one run — copied, verified, and only then trashed — so **Undo this run** puts every file back at its original path, even after you have emptied the Trash. Anything too large for the capsule to protect is **left undeleted rather than deleted unprotected**, said in the manifest before you click rather than in a summary afterwards. (Undo restores content byte for byte; a restored file's *date modified* reads as the moment it came back, which the dialog says.)
