...
- **The native scan engine lists a whole folder in one system call.** On macOS it asks `getattrlistbulk` for every entry's name, type, sizes, times and flags at once, on its own worker threads (Linux: `getdents64` + `statx`; Windows: `FileIdExtdDirectoryInfo`), and hands the tree over as columns into the same store — so the JSON is the same, and the equivalence tests hold it byte-identical to the built-in walker's (verified on macOS; Linux and Windows run on CI). Recorded by `npm run bench` on an Apple M3 (4 performance + 4 efficiency cores, 16 GB, macOS 27), warm cache, Turbo budget, load 2.2–3.6 (up to 7.0 during the built-in walker’s runs on the 1,000,000-entry tree), 23 September 2026: **400,417 entries/s on a 200,000-entry tree** (the built-in walker: 131,203), and **91,308 entries/s on a 1,000,000-entry tree** too big for the kernel's metadata cache, which reads 2.3 GB per scan (the walker: 82,497). At the Eco budget it runs on the efficiency cores and its speed varied too much run to run to record (±17%). Where the native module cannot load, the built-in walker runs and the scan's stats say why.
