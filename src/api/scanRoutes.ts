...
export function buildScanStats(scan: ScanResult): ScanStats {
  return {
    ...
    sparseFiles: scan.sparseFiles ?? 0,
    sparseBytes: scan.sparseBytes ?? 0,
    slackBytes: scan.slackBytes ?? 0,
...
scanRouter.get('/scan/:scanId/budgets', async (req: Request, res: Response) => {
  ...
  for (const b of budgets) {
    ...
    if (node === -1 || !store.isDir(node)) continue;
    out.push({
      path: b.path,
      name: store.name(node),
      maxBytes: b.maxBytes,
      actualBytes: store.size(node),
      overBy: store.size(node) - b.maxBytes,
    });
  }
