...
async function loadTrendData() {
  ...
  // A new root's forecast is unknown until the server answers, and unknown
  // never projects — the dashed tail is a claim, not a default.
  state.trends.forecastOk = false;
  drawTrendNet();
  ...
  drawTrendChart();
  renderTrendDeltas();
  // The chart's dashed line is a plain linear fit; the breach date beside it
  // comes only from the server's gated forecast, which refuses to guess when
  // the history is thin, erratic or shrinking — so the label never claims
  // more than the fit stands behind. Fetched after the paint: it is garnish.
...
async function labelTrendForecast(path, seq) {
  ...
  // The gate for the chart's dashed tail is this same status — one policy,
  // one reader — so the answer repaints the chart before anything else.
  const ok = f.status === 'ok';
  if (state.trends.forecastOk !== ok) { state.trends.forecastOk = ok; drawTrendChart(); }
 ...
 *
 * `allowed` is the app's ONE honesty gate — the server's own forecast status.
 * The dashboard refuses to name a disk-full date until that gate says 'ok',
 * and this chart obeys the same answer rather than quietly drawing a fit the
 * rest of the app is declining to stand behind.
 *
 ...
 */
function trendProjection(pts, allowed) {
  const MAX_MS = 30 * 864e5;      // a month is as far as this fit is worth reading
  const SPAN_FRACTION = 0.5;      // → real data holds ≥ 2/3 of the x-domain
  if (!allowed || pts.length < 3) return null;
  const last = pts[pts.length - 1];
  const span = last.t - pts[0].t;
  if (!(span > 0)) return null;   // no elapsed time, no rate to extend
  const horizon = last.t + Math.min(MAX_MS, span * SPAN_FRACTION);
  const fit = FxCharts.math.linreg(pts.map(p => ({ x: p.t, y: p.v })));
  return { name: 'Forecast', points: [
    { t: last.t, v: last.v },
    { t: horizon, v: Math.max(0, fit.project(horizon)) },
  ] };
}
...
function drawTrendChart() {
  ...
  const projection = trendProjection(pts, state.trends.forecastOk === true);
