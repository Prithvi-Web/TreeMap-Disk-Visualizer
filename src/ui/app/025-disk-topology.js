...
function renderTopology(topo) {
  ...
  // A disk that is a member of a pool holds its share of the pool's volumes,
  // which the combined section below shows; "no volumes" would be false of it
  // (issue #35 was filed over that very sentence).
  const pooledIds = new Set();
  for (const key of groups.keys()) { if (key.includes('|')) for (const id of key.split('|')) pooledIds.add(id); }
...
function topoSection({ title, tag, capacity, vols, inPool = false }, rank = 0) {
  ...
  const caveat = !vols.length
    ? `<div class="topo-note">${inPool ? 'Part of the pool below.' : 'No volumes on this disk.'}</div>`
    : !known.length
