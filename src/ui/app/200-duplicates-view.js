...
async function loadDuplicates(force = false) {
  ...
  const poll = async () => {
    ...
    try {
      ...
      state.dup.notHashed = data.notHashed;
...
async function freeSpaceNow() {
  ...
  const root = state.root && state.root.path;
  // Containment EITHER WAY. /api/system measures the home folder's volume, so
  // the question is whether the scanned tree is on that volume — and a scan of
  // /, /Users or C:\ is, while failing a one-directional startsWith(home).
  // Those are among the commonest roots anyone scans.
  if (!home || !root || !(root.startsWith(home) || home.startsWith(root))) return null;
 ...
 *
 * The cloud line appears whenever the hunt left files unopened because
 * opening them would download them (the master prompt §3.2: never silently
 * omit them) — with or without a figure, since "No duplicates found" over
 * unchecked files would be a claim nobody made.
 */
function dupNoteLines(platform, status, groupCount, outcome, notHashed) {
  const lines = [];
  ...
  if (status === 'complete' && notHashed && notHashed.files > 0) {
    const one = notHashed.files === 1;
    lines.push(`${formatCount(notHashed.files)} cloud ${one ? 'file' : 'files'} (${formatBytes(notHashed.bytes)}) ${one ? 'was' : 'were'} not checked, because opening ${one ? 'it' : 'them'} would download ${one ? 'it' : 'them'}. To include ${one ? 'it' : 'them'}, make ${one ? 'it' : 'them'} available offline first.`);
  }
...
function renderDupNote() {
  const el = $('dupNote');
  if (!el) return;
  const html = dupNoteLines((state.system || {}).platform, state.dup.status, state.dup.groups.length, dupTrashOutcome, state.dup.notHashed)
