...
function buildFxTree(): {
  root: FileNode;
  stats: ReturnType<typeof buildNtfsMftStoreFromEdges>["stats"];
} {
...
test("sums sizes, deduping the genuine hardlink exactly once", () => {
  ...
  assert.equal(
    find(root, "hardlink.txt")!.size,
    0,
    "second occurrence is zeroed",
  );
...
test("parseNtfsMftEdgesDetailed reads _meta targetRecordNo and JSON unicode escapes", () => {
  const nd = [
    ...
    '{"recordNo":43,"parentRecordNo":42,"name":"a\\u202eb","size":1,"isDir":false,"mtimeMs":0}',
  ].join("\n");
  const { edgesByParent, targetRecordNo } = parseNtfsMftEdgesDetailed(nd);
  assert.equal(targetRecordNo, 42);
  assert.equal(edgesByParent.get(42)![0].name, "a\u202eb");
});
