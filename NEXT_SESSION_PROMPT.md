...
### Where things stand
- **Remote `main` = `32d68ff`; local `main` is 70 commits ahead (13:25 UTC; the owner has not pushed yet)** (`git log --oneline origin/main..main`). The owner pushes from GitHub Desktop. CI watcher: `Monitor` on `scratchpad/watch-ci.sh` (waits for `git ls-remote` to move off `scratchpad/remote-base.sha`, then reads each leg and its annotations); re-arm it every 30 min.
...
- **Equivalence at scale (13:05 UTC, `4ccaadb`):** native vs walker on the enum200k bench corpus through the gate's own child (`scratchpad/equiv-enum200k.ts`): 200,000 nodes each, **0 lines differ outside accessedAt** (set aside: two runs over one tree move directory atimes), **all 11 counters agree**; native 456 ms vs walker 1,817 ms.
- **`4ccaadb` (12:57 UTC):** Settings' NTFS turbo row and the OpenAPI `engine` description say up front that the mode needs TreeMap installed for everyone who uses the computer (the sentence is pinned by `tests/settingsNtfsMft.test.ts`).
- **Gate at the tip (last full run 12:57 UTC, on `4ccaadb`; only docs since):** full `npm test` 2,932 / 2,925 pass / 0 fail / 7 skipped; `cargo test --workspace --no-fail-fast` 329 / 0; clippy -D warnings on macOS, x86_64-unknown-linux-gnu, x86_64-pc-windows-msvc; typecheck; build-ui --check.
