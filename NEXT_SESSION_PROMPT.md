...
### Where things stand (rewritten 23 Sep 2026, ~16:10 UTC)
...
1. **RT2 (red team):** the app asks even for a drive the helper will always refuse (mapped network drive, removable, non-NTFS). Plan: in `tm-mft/src/win32.rs` split `read_volume_with`'s unprivileged checks (`check_root`, `volume_path_name`, `device_path`, `check_drive_type`, `volume_information` + `check_file_system`; NOT `root_identity`, which may be exactly the folder only an administrator opens) into `precheck_with(api, root)`; Windows wrapper `precheck`; napi `mftPrecheck(root): string | null` (null off Windows); `runMftWalk` calls it before the prompt (a rule: `failed: false`). A `subst` drive passes these checks and fails elevated (per-session letters) — E's switch-off bounds it to one prompt; `QueryDosDeviceW` could catch it.
2. **RT3 (red team + maintainability):** a helper output left behind when TreeMap quits mid-run is never swept (`removeQuietly`'s comment claims a sweep that does not exist). Plan: before asking, remove `*.tmmft` in the app temp folder older than an hour that no scan of this session is using.
3. **Columns-file hardening:** `check_shape` should refuse non-finite or negative sizes/allocations (adversarial #8) and names that are empty, `.`, `..` or hold `/`, `\`, `:`, NUL (security, conf 4; `tree.rs` refuses them for directories only — skip them as unreadable for files too); `decode_stats` should refuse counts past 2^53−1 or past the entry count, and a non-finite `wall_ms` (API contract: a u64 over 2^53 becomes a BigInt through serde_json and throws "Cannot mix BigInt" in the ingest).
4. **U:** `build-native.js` returns `installed: []` when a later rename fails after the module's succeeded, so VERSION is not rewritten beside a replaced module (TS + maintainability).
