...
### Where things stand
- **Remote `main` = `32d68ff`; local `main` is 56 commits ahead** (`git log --oneline origin/main..main`). The owner pushes from GitHub Desktop. CI watcher: `Monitor` on `scratchpad/watch-ci.sh` (waits for `git ls-remote` to move off `scratchpad/remote-base.sha`, then reads each leg and its annotations); re-arm it every 30 min.
