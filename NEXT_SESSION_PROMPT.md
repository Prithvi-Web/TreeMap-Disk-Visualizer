...
- DONE `13d717c` baseline after T1 (275,734 e/s). DONE `2a50111` **streamed mtime cache**: `scan.root` (a 200k-node object tree, 69 ms) + `JSON.stringify` (48 MB, 80 ms) ran synchronously AT completion; now `streamTreeJson` (src/services/scanStoreJson.ts) + `writeFileChunked` (storage.ts) write the same bytes in chunks. Bench PASS 12.9% (725.3 → 631.7 ms), peak RSS 271.7 → 159.6 MB; 17 mutants red; independent review's MEDIUM (failing close masked the producer's error) fixed. DONE `f16d48a` baseline after it (316,315 e/s, 632.3 ms). DONE `a51e152` **NATIVE_POLL_MS 100 → 10**: slack 19–77 → ~5 ms; ci20k 99 → 72 ms (stage diagnostic under load).
- **FOUND — bench harness defect (agent "bench-fix" in flight, owns `bench/**` + `tests/bench*.test.ts`, told not to commit):** every measured child compiles the macOS usage probe (xcrun clang) right before its timed window, evicting corpus metadata from the vnode cache: the same enum200k scan measured 529 ms without it vs 634 ms with it (walk 408 → 468 ms; the bench reported 632). Every "warm" baseline so far carries this. Fix: build the probe once per invocation, before the warm-up. The agent also fixes the 3 queued harness items (recordBaseline refusal, dirty check fails closed, compare conditions for --min-size/--threshold). **When it reports: verify, commit, run the FULL `npm test`, then re-record ALL affected baselines from a clean tree and say plainly in the commit that the old ones were depressed by the harness, not the app.**
- Stage-attribution tools (scratchpad, not committed): `attribute-app.ts` (per-stage timings of one startScan in a fresh process, cold + warm), `stage-preload.mjs` (the same, preloaded into the bench's own worker), `profile-ingest.ts`, `ablate-*/run.ts`. Ingest ablation (64 ms): name sort 12.6, store insert 15, statToInput 14, hardlink Map.get 5 (→ typed array: −5 ms, byte-identical, NOT yet committed), utf8 decode 5. Faster comparators do not help (call overhead); removing the sort needs the walker to emit folders pre-sorted (Rust contract change) — a lead, not started.
- **NEXT (in order):** (a) land the bench-fix agent's work as above; (b) the hardlink typed-array ingest change (test: output hash identical; −5 ms); (c) quiet-machine bench of enum200k/ci20k native Turbo + enum200k native Eco for the Phase 3 table; walk-only rate is ~485k e/s (inside Tier B's 400–700k), whole scan ~390k after the fixes — say both; (d) harness follow-ups (now inside (a)) (queued list below); (c) CURRENT-STATE §11 + DESIGN: the Phase 3 numbers and the attribution; README only with bench-recorded numbers and conditions; (d) W6 M6 (helper, cross-check that ignores entries newer than the read, `FastPath::Mft`, the setting, Electron's prompt) or ship W6 behind the setting labelled `not verified on this build`; (e) the Phase 3 check-in (plain English, the table of measured-vs-target); then Phase 4.
