...
### LATEST (24 Sep 2026) — read first
- **Remote `main` = `ce805ea`** (Phase 3's close-out, pushed by the owner). Its CI run 35950892207 failed three tm-governor timing tests (macOS: "throttle() never sleeps: 262 ms for 100 ms of work"; Windows: a 50 ms spin showed no CPU; Windows: the 19% hold held 11% with both workers at full duty). All three read the wall clock on a busy runner. **Fixed in the commit after `ce805ea`** (`fix(governor): …count what the governor does…`): `ThrottleTotals { worked, owed, requested, slept }` + `since()`, the ledger as pure `ledger_charge`/`ledger_settle`, the throttle test asserting the duty law on counts and waiting by ticks, the sampler test spinning until the CPU clock shows 10 ms, `HoldReport.machine_idle_last_half` with the hold waiting for room (≤ 60 s) and holding again (3 attempts) only when the machine had no idle CPU. Verified under 4 and 10 busy loops (`scratchpad/load-run.sh`, `suite-under-load.sh`), every governor mutant red, `ecc:rust-reviewer`'s two findings fixed. **Waiting for the owner's push; Phase 3 closes when that run is green on all four legs.**
- **Phase 4 S1 (`tm-store`) is written and verified in `scratchpad/ws-store`** (a copy of the workspace, so the CI-fix commit stayed clean): 23 tests, 45 mutants red, clippy on three targets. It moves to `native/treemap-core/crates/tm-store` after the CI-fix commit. Plan amendments are in `docs/superpowers/plans/2026-09-18-phase4-storage.md` ("Amended in S1"; `Math.round` corrected — `(x + 0.5).floor()` is wrong at 0.49999999999999994, 2^52+1 and [−0.5, 0)).
- **Next: S2** — `scanTakeStore(handle, opts)` in tm-node moving the store's Vecs (at capacity) into typed arrays; `PackedScanStore.adoptColumns` (same object, so diskScanner is unchanged); `containerKind.ts` as a rule table (one source for both sides); the Node passes over `textCandidates` and `cloudCandidates`; the differential test (the same fixture through `ingestColumns` and through the Rust store → byte-identical pruned JSON and counters). **Found for S3/S5:** a `WalkOutput` is ~60 B/node before the store (~6 GB at 100M), so the 100M ceilings (1.5 GB spill, 400 MB aggregate) cannot be met by building from a whole walk: the walk must stream into the store — to be designed and measured in S3, and reported as a correction to the plan.
