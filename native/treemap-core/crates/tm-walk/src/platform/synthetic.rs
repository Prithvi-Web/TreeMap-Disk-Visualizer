...
use crate::platform::{ListBuffer, Lister, Meta, time_ms};
...
impl Shape {
    fn new(spec: &SyntheticSpec) -> Result<Self, String> {
    ...
    fn child_folders(&self, g: u64) -> Range<u64> {
    ...
    fn parent_of(&self, g: u64) -> Option<u64> {
...
impl Lister for SyntheticLister {
    ...
    fn list(
        ...
        for child in self.shape.child_folders(folder) {
            checkpoint(buf, listed)?;
            self.dir_name(child, &mut name);
            buf.listing.push(&name, self.dir_meta(child, want_atime));
            listed += 1;
        }
        let first = self.shape.first_file(folder);
        for k in 0..self.shape.files_in(folder) {
            checkpoint(buf, listed)?;
            let q = first + k;
            self.file_name(q, k, &mut name);
            buf.listing.push(&name, self.file_meta(q, want_atime));
            listed += 1;
        }
