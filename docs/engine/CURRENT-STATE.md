...
* Both the mtime cache (a full tree, JSON) and `scan.root` (a getter that prunes the store with `maxNodes: MAX_SAFE_INTEGER`, `diskScanner.ts:307-322`) still materialise object trees; the cache is capped at 300k nodes for that reason.
  * *Changed 23 September 2026 (Phase 3):* the mtime cache no longer does. It is streamed from the store (`src/services/scanStoreJson.ts`, `streamTreeJson`) through `writeFileChunked` — the same bytes as `JSON.stringify(scan.root)`, pinned by a test, without the object tree or the whole string. Measured on enum200k (native, Turbo, warm, M3): rebuilding the tree had cost 69 ms and stringifying it (48 MB) 80 ms, both on the event loop as the scan completed; the bench's wall time fell 725.3 → 631.7 ms and peak RSS 271.7 → 159.6 MB. `scan.root` itself still materialises when read. The 300k cap stays: it now bounds the file and the rescan that parses it.
...
Governor holds with a **live scan as the load** (Phase 3's gate, 24 Sep 2026): Eco 22.2 % (p95 25.1 %, highest 25.7 %; 18 scans), Balanced 26.7 % (p95 38.7 %, highest 39.2 %; 83 scans), Turbo 35.1 % (p95 51.4 %, highest 52.1 %; 101 scans) — the mean of each 60 s hold's last half, enum200k scanned back to back under the preset (`npm run bench -- scanhold`), recorded at `77152a5`, load 2.35–2.49. A scan waits on the disk, so it is held from below — at most the ceiling and its 5-point band — where the synthetic hold must sit at the ceiling; Eco's samples cluster at 24.5–25.1 %, the governor holding the walk at its ceiling, while Balanced and Turbo scans never reach theirs.
