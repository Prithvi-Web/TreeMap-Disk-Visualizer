...
| D2 | **Keep every existing engine**, unchanged, as the fallback chain (`gdu-turbo` → `walker`; on Windows `walker` alone since 23 Sep 2026, RISKS R59) and as the correctness oracle | Prompt rules 3 and 13; the equivalence test needs an oracle | — |
...
| D9 | Work lands on `main` in small commits, one phase at a time, as every TreeMap session has; the owner pushes | The owner's standing workflow; the prompt's "branch per phase" was offered as the alternative | **Decided: `main`**, 18 Sep 2026; on 23 Sep 2026 the owner asked for a push after each fully gated step; the agent's command line holds no GitHub sign-in, so the agent commits each step and the owner pushes from GitHub Desktop, and the owner alone publishes a release (never a `v*` tag from the agent) |
...
**Added 24 September 2026 from the Phase 4 design (plan §S.2); items 11–14 were designed, not built. Item 11 is built (25 September 2026, T1); items 12–14 are not built:**

11. **`notHashedReport` breaks ties by path, in every mode (T1; built 25 September 2026).** It lists the 20 largest placeholders the duplicate pass left unread; which of several equal-size placeholders at the cut make the list must not depend on how the store numbered them, because block numbering (§6) follows the walk's schedule.
