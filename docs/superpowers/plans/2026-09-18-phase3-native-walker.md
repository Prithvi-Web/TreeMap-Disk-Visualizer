...
| W2 scan API in `tm-node`, `nativeEngine.ts`, selection, stats, settings, golden re-record | **done** — committed `f64c287`; 31 + 9 + 1 tests, 10 mutants red; child order was the one real defect (libuv sorts, APFS hashes); golden re-recorded with the ten stats keys; measured on a 20,401-entry fixture: walker 275,689 entries/s, native 850,042 (3.1×), warm and busy Mac | `src/services/scan/{nativeEngine,nodeInput}.ts`, `crates/tm-node/src/lib.rs` |
| W3 canonical digest, edge-case fixture, equivalence and fallback tests (the bench `native` engine choice moved to W2) | **done** — committed `928257c`; 9 + 17 + 9 tests, 16 mutants red (2 recorded as not reddenable here: the families rule and the corpus warm-up); walker × 2 identical on smoke (1,200 nodes), ci20k (20,000) and the 50-node edge fixture; gdu vs walker identical apart from `accessedAt` (gdu records none — DESIGN §16 item 1 amended) with all 11 counters equal; native vs walker identical on smoke and ci20k, and on the edge fixture after `0a0ea2e` (a mount point's attributes — the gate's first real finding, fixed in the macOS lister); 13 of 14 edge cases built here (hdiutil mounts included), the case-collision pair skipped on case-insensitive APFS; the forced-load-failure case names the path in `fallbackReason` and digests the same | `tests/fixtures/{canonicalTree,edgeCases,equivalenceChild}.ts`, `tests/{canonicalTree,edgeCases,nativeEquivalence}.test.ts` |
| W4 Windows listing (`FileIdExtdDirectoryInfo`, `FindFirstFileExW` fallback, long paths) | **done, unproven live** — committed `82289c1` with W5; 16 synthetic-buffer tests, both cross targets check and clippy clean; the file-id collision rule lives in `walk.rs`; eight questions only the Windows CI leg can answer (see the W4+W5 report in HANDOFF/NEXT_SESSION_PROMPT) | `crates/tm-walk/src/platform/windows.rs`, `tests/windows_parse.rs` |
| W5 Linux listing (`getdents64` + `statx`) | **done, unproven live** — in `82289c1`; 13 synthetic-buffer tests; `statx` masks on FUSE/NFS/overlayfs and the `ENOSYS` switch are the Linux CI leg's to prove | `crates/tm-walk/src/platform/linux.rs`, `tests/linux_parse.rs` |
| W6 Windows MFT turbo (D7, opt-in, elevated helper, cross-check) | **done, unproven live** — M1–M6 (`9c301a9`, `28b2830`, `8bbd8c8`, `b4dc6dc`); its own plan's table has the detail; the UAC prompt carries W6-9's `not verified on this build` | `docs/superpowers/plans/2026-09-23-phase3-w6-mft.md` |
| Rust review + fix round | **done** — 22 Sep 2026, commits `3b3c18e` (walker) and `95d5a0e` (the app). Four read-only reviewers over `tm-walk`, the two cross-platform listings, the scan bindings and the fallback path; every finding verified in the code before it was fixed. Five defects in the walker (a worker panic wedged the walk; `next_id` wrapped; a path-shaped directory name reached `Path::join`; a withheld `OBJTYPE` dropped a subtree; no cancel or heartbeat inside a listing) and four at the boundary (`scanTake` joined the driver on Node's main thread — a wedged mount froze the whole app; no stall deadline; a leaked handle on a mid-walk throw; a forced `native` setting that said nothing when it was overridden). Plus `MAX_BUFFER_BYTES`, Linux `EPERM` on `statx`, negative Windows sizes, NUL in a name and in a path. Every behaviour reddened by a mutant; R53–R57 record what was deliberately not changed | `crates/tm-walk/**`, `crates/tm-node/src/lib.rs`, `src/services/scan/nativeEngine.ts` |
| Gate | **passed 24 Sep 2026** — equivalence digests identical on every corpus and the edge fixture on all four CI legs (macOS, Windows, Linux, Linux pt-BR: run 35947322361 on `abc5c2c`, green end to end, the Windows and Linux listings' and W6's first complete live proof); the enumeration numbers against the Tier B targets in CURRENT-STATE §11.2 with every miss stated with its measured figure and reason (Turbo met: enum200k 412,291 entries/s at `17eb13d`; Eco and CPU-seconds per million missed, measured); the governor bands held with a live scan as the load, each under its ceiling and band: Eco 22.2 % (p95 25.1 %, highest 25.7 %; 18 scans), Balanced 26.7 % (p95 38.7 %, highest 39.2 %; 83 scans), Turbo 35.1 % (p95 51.4 %, highest 52.1 %; 101 scans) — the mean of each 60 s hold's last half, enum200k scanned back to back under the preset (`npm run bench -- scanhold`), recorded at `77152a5`, load 2.35–2.49; full gate green. The Windows/Linux throughput targets are unmeasured on a tier machine (the owner has a Mac only; CI runners are no tier) | `bench/baselines/governor-native-scan-*` |
...
| P3-7 | **Sizes and allocation cross the boundary as `Float64Array`** (exact to 2^53), `dev` and `ino` as doubles too — Node's own `Stats` uses doubles, so the hard-link key `${dev}:${ino}` is the legacy key exactly. **Amended 23 Sep 2026 (pre-landing review): ids no longer cross at all.** A double holds an id exactly only below 2^53, and a reused NTFS record's id passes it (its sequence number sits in bits 48..64), so neighbouring files became one family on Windows. The walk now groups names by each file's whole id (`u128`) and hands over `hardlinkFamily: Uint32Array`; the walker reads an id past 2^53 again as a bigint. | Legacy-identical keys; no BigInt in the hot loop. (Amended: exact keys; still no BigInt in the hot loop — the walker's second look is per shared file past 2^53 only.) |
...
interface WalkResult {
  ...
  hardlinkNode: Uint32Array; hardlinkFamily: Uint32Array;   // was hardlinkDev/hardlinkIno as doubles (P3-7, amended 23 Sep 2026)
