...
| W3 canonical digest, edge-case fixture, equivalence and fallback tests (the bench `native` engine choice moved to W2) | in flight — interrupted once and resumed; on disk: `tests/fixtures/{canonicalTree,edgeCases,equivalenceChild}.ts`, `tests/{canonicalTree,edgeCases,nativeEquivalence}.test.ts`; last run 5 passed / 4 skipped-with-reason (native cases wait for W2); walker twice identical on smoke, ci20k and the edge fixture; gdu vs walker identical on 1,200 and 20,000 nodes apart from `accessedAt`, which gdu cannot record | |
