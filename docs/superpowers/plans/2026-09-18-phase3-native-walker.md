...
| Gate | **not run** — remaining: the two CI fixes (fmt on `tests/walk.rs`, the Linux-only clippy in `tm-governor/src/sample.rs:222`), the owner's push and the four CI legs read, a Rust review of `tm-walk` + the scan bindings with its fix round, `npm run bench -- enumerate --engine=native` on ci20k / enum200k / enum1m with `--record`, the three governor baselines, W6 (MFT), the README/PLATFORM_NOTES sentences, the check-in | |
