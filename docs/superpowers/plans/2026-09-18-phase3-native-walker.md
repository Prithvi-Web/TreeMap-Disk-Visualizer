...
| W2 scan API in `tm-node`, `nativeEngine.ts`, selection, stats, settings, golden re-record | in flight — interrupted once by a usage limit and resumed; on disk: the whole TypeScript side (`src/services/scan/{nativeEngine,nodeInput}.ts`, selection and stats in `diskScanner.ts`/`scanRoutes.ts`, settings + UI row, bench `native` choice, `native/index.d.ts` declarations), typecheck clean; the Rust bindings in `crates/tm-node/src/lib.rs` and the real-module tests still to come | |
...
| W4 Windows listing (`FileIdExtdDirectoryInfo`, `FindFirstFileExW` fallback, long paths) | in flight with W5 in one implementer (one owner for `platform/mod.rs`) — interrupted once and resumed; on disk: `tests/windows_parse.rs` (red) | |
| W5 Linux listing (`getdents64` + `statx`) | in flight (see W4) | |
