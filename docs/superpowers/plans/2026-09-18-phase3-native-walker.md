...
| Gate | **not run** — remaining: the owner's push and the four CI legs read (the first live proof of `windows.rs` and `linux.rs`; the eight questions are the reading guide), `npm run bench -- enumerate --engine=native` on ci20k / enum200k / enum1m with `--record` plus the three governor baselines, W6 (MFT), and the Phase 3 check-in. The two CI fixes (`6983dae`) and the README/PLATFORM_NOTES sentences (`c4bdd8e`) are **done** |  |
