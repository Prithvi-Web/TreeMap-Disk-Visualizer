...
| Rust review (ecc:rust-reviewer over both crates) | **done** — CRITICAL: `SyntheticLoad::stop()` hung while the governor was paused (workers parked in `throttle()` could be released only by the governor); HIGH: a dropped load kept spinning; both fixed red-first (`Governor::throttle_unless`, a `Drop` on the load, the walker's pacer throttling with its own cancel — 4 mutants red). MEDIUM (a `governorConfigure` landing mid-`governorHold` is not reflected in the report) is fixed in tm-node with W2's bindings. LOW (workers and duty published as two atomics, so a reader can pair one tick's count with the previous tick's duty) is accepted: the pair self-corrects at the next tick and a caller reads the two at different instants anyway, so packing them would not close the window it names | `tests/loadgen.rs`, `tests/walk.rs::live::cancel_completes_while_the_governor_is_paused` |
