...
| P4-8 | **The 100M gate runs on a synthetic listing, not a disk.** `tm-walk`'s `Lister` trait gets a scripted implementation (`SyntheticLister`: fan-out, depth, size distribution, seed) reachable from the napi surface behind `scanStart(root, { synthetic: {...} })` for `root` under the app's own temp directory, and the bench harness's `enumerate --corpus=synthetic100m` measures it in a child process like any other run, labelled **`source: synthetic` in every report and refused as a throughput baseline** (it lists no file system). **Clarified 25 Sep 2026 (T3, built):** "refused as a throughput baseline" means a synthetic result is never compared with a file-system run: `bench compare` answers NOT COMPARABLE for the mix, in either direction, and a synthetic result never replaces a file-system baseline recorded under its name. Two synthetic results compare with each other, and T22 records them with `--record` under their synthetic names. Peak RSS is the harness's measurement, not the app's. | 100M real entries do not fit on this Mac (the 1M corpus is 34 GB); the ceiling being gated is the store's, and a synthetic listing exercises exactly the store. |
...
**Presets.** They create nothing on disk and are labelled `source: synthetic`; `bench compare` refuses them as a throughput baseline (never compared with a file-system run; two synthetic runs compare, P4-8's clarification of 25 Sep 2026).
...
- [ ] **T0. Documents only.**
  ...
  - **25 Sep 2026:** the owner decided Q3 (the recommendation is taken: the 10M row is met by spill, memory mode up to 5M). Q1 and Q5 are still pending the owner, and Q2, Q4 and Q7 still await the owner's confirmation.
- [x] **T1. `notHashedReport` breaks ties by path.**
  ...
  - **Built 25 Sep 2026.** One comparator (size, then the smaller path by `<`: UTF-16 code units, no locale) decides both which placeholders make the list and their order; paths are built only for ids at least as big as the 20th largest, into a sorted buffer of at most 20 (`keepIfNamed`). `tests/duplicatePlaceholders.test.ts`: five tests (the equal-size fixture tells `<` apart from id order, `localeCompare` and UTF-8 byte order; the tie at the twentieth place; a too-small placeholder's path never built; distinct sizes met smallest first; at most twenty names held). The review's four findings: the floor's coverage gap and the unbounded tie buffer repaired, the two path orders refuted by both skeptics, the attribution handled by committing the isolation sweep first. 17 mutants red across the build and the repair.
- [x] **T2. The guards, before anything moves.** **Built 25 Sep 2026:** `tests/fixtures/renumber.ts` (`renumberStore(source, seed, {schedule: random | depthFirst | breadthFirst, folderTotals: kept | zeroed})` gives each folder's children one block and adopts the result through `adoptColumns`; it refuses tombstones, cloudIds and logicalSizes, which have no column; `checkBlockInvariants` names the invariant a column breaks) and `tests/numberingIndependence.test.ts`: the battery (13 trees × 3 renumberings, about 60 answers each, folder totals zeroed and re-summed; one tree is on disk, so duplicate groups that tie on `reclaimable` and copies that tie on mtime are compared), the whole golden capture over HTTP and MCP under three renumberings, the native golden leg in memory mode (green on today's code: no pre-existing difference), and the 1-against-8-worker determinism leg. The review's three findings (groups never formed off disk; the logicalSize refusal untested; the I3 line unreached) and two more unreached checker lines found by the repair are fixed, each with its mutant red. Left for later tasks: the forced-spill golden leg (T13–T15), the 2- and 64-worker legs under a random-yield pacer (T6), the checker on native-produced stores (T6).
...
- [x] **T3. `SyntheticLister`** (moved forward because everything after it measures with it). **Built 25 Sep 2026:** `tm-walk` `platform/synthetic.rs` (`SyntheticSpec` with integer fields, `SyntheticSpec::developer`, `SyntheticLister`, `synthetic_temp_folder()`), `WalkOptions.synthetic`, `WalkError::OptionsRefused`, `lister_for`; `scanStart`'s `synthetic` option (unknown keys refused; shares as whole parts per million) and `syntheticTempFolder()`, because Node's `os.tmpdir()` does not follow Rust's `temp_dir()` rule; the `synthetic10m`/`synthetic100m` presets (nothing on disk; counts by the walk's own arithmetic); bench results carry `source`, and a synthetic result is never compared with a file-system one. The native contract is 0.2.0 (package.json `nativeVersion`, the workspace version), because `native/index.d.ts` changed shape (native/README.md, "The version handshake"). 17 Rust tests, 11 Node tests; 25 mutants at build and 8 at the review's repair, all red. The end-to-end `bench -- enumerate --corpus=synthetic10m` needs T21's plumbing (`measureWorker.ts`, `bench/run.ts`). A synthetic walk reports `fastPath: 'unavailable'`: a `'synthetic'` label needs a new code in tm-mft's columns encoding.
...
- [x] **T4. The digest lock:** committed column digests of `build(take())` for the scripted and synthetic fixtures at 1 and 8 workers.
  - **Green gate:** recorded on a clean tree.
  - **Built 25 Sep 2026:** `crates/tm-store/tests/digest_lock.rs`. Eight fixtures walked through the real walk (`start_with` → `take()` → `build`) at 1 and at 8 workers, which must agree: scripted-posix (the Lemma 3 family whose walk-order-first, breadth-first-first and smallest-path members all differ; `'a'`+0xF8 against `'a😀'`; every derive and container rule; refusals; sparse, slack and placeholder files; 100 levels), scripted-windows (listing order kept, id families refreshed through the fake lister, ids differing above bit 64), scripted-wide (65,605 files in one folder, the extension dictionary overflowing, sparse terms past 2^53), and five synthetic trees of 30,001 rows. Every `Store` column, side table, the `Counters` and the tree facts of `WalkStats` are digested (FNV-1a 64, floats by their bits; the structs are destructured without `..`, so a new field fails to compile until it is digested); a failure names the fixture and column that moved and prints the whole new table, which is never re-recorded automatically. One table for every platform: no fixture lists a disk, and names are keyed by their lossy spelling (a Unix-only test pins it) and may not hold `\` or `:`. Reviewed by ecc:rust-reviewer (no critical or high; its two findings fixed). 16 mutants red (derive, build, finalize, synthetic and walk rules; the key spelling; a separator in a name). **T6 runs this lock under both numberings, through `walk_and_build()`, and must not re-record it.**
- [x] **T5. `Column::Anon`** (mmap on POSIX, `VirtualAlloc` reserve and commit on Windows) and `PackedScanStore.release()`'s guard. **Built 25 Sep 2026:** `Column::Anon(AnonRows<T>)`, `Column::anon(len, capacity)`, `with_headroom()`, the `unsafe trait Zeroable`, a per-thread `anon_tally`, `ColumnError` (`LenPastCapacity`, `TooLarge`, `MapFailed`); `PartialEq` compares rows only. Windows declares `VirtualAlloc`/`VirtualFree` by hand (`#[link(name = "kernel32")]`), so no windows-sys feature is enabled; tm-store gains `libc` on unix only (a crate already in the lock). `PackedScanStore.release()` drops every column and side table and every accessor throws afterwards (`live()`). Kernel probes: `msync` on POSIX, `VirtualQuery` on Windows. 13 mutants red here; the three Windows-only mutants (VirtualFree never called, `MEM_DECOMMIT`, heap-backed rows) compile but can only be seen red on CI's Windows leg. Nothing uses either yet: T7 wires `Column::Anon`, T17 calls `release()`.
  - **Tests first:**
...
- [ ] **T6b. Bound what T6 could not** (found by T6, 26 Sep 2026; must land before T12, whose budget depends on both bounds):
  - **Step 1 built 26 Sep 2026** (`18c74d6`): `walk.rs` (1,460 lines after T6) split into `walk/{shared,merge,discovery}.rs`, a pure move checked block by block; every file under 800 lines.
  - **Range jobs:** a folder's subfolders are queued as one job per contiguous range of the folder's block, split as workers take from it, ~~so the queue and the open set become O(workers × depth) whatever the fan-out~~. The cost (review of T6): a range job must re-derive each remaining child's name from the parent's committed block, so the parent's names stay resident (or re-readable) until the range drains — measure it against the jobs it replaces.
    - **Built 26 Sep 2026 (step 2):** one `Range` per committed listing holds the parent's path once and, per subfolder, its id, its name's end and its OS name (never the lossy stored name); a worker's `DirJob` is built when it takes the folder. **Scheduling is T6's, folder for folder** (the hybrid rule compares `q_max` with the folders waiting; FIFO takes the front range's first remaining folder, LIFO the back range's last), so every output and T4's lock are unchanged. **Measured:** a waiting folder costs its name + 8 B (26 B with 18-byte names, against T6's ~200 B per job); 1M empty subfolders cost +15.3 MB of peak RSS instead of +172 MB. `WalkCounts` gains `ranges_peak` and `queue_bytes_peak`. Each range keeps the parent block's first id for T7's position paths.
    - **The bound, corrected (step 2's exhaustive model, 2,070 shapes):** with one worker the ranges queued are at most `q_max + D − 1`, proved and tight. With several workers **no bound in W·D holds under every schedule**: workers that stall one after another on single folders leave partly taken ranges under each other's descents, and the model's worst case is `q_max − 1 + C(D + W − 1, W)` (polynomial of degree W in D; millions for 8 workers at depth 20). By the same mechanism T6's folder bound `q_max + W·(D+1)·f` fails at depth too. So range jobs fix the cost per waiting folder, not the count of open parents; **T6c** bounds that.
  - **Listers that stop at a batch size:** `Lister::list` becomes resumable (a partial listing plus a continuation; macOS `getattrlistbulk`, Linux `getdents64` and Windows `FileIdExtdDirectoryInfo` each expose a cursor), and the big-listing gate is consulted between batches, so at most one big listing is resident.
    - **Built 26 Sep 2026 (step 3):** `Lister::list_until(dir, want_atime, buf, limit)` answers `Complete` or `More`, and `list_more` reads on; the open descriptor or handle and the listing's place live in the `ListBuffer` as its cursor, closed however the listing ends. Every lister resumes: macOS `getattrlistbulk` and its per-entry fallback, Linux `getdents64`, Windows `FileIdExtdDirectoryInfo` and its `FindFirstFileExW` fallback, the synthetic lister, and the tests' scripted listers (which now answer in batches). Listers stop at exactly `limit`, inside a batch if they must (the batch's rest is kept with the cursor), which tightens the bound. The mount-point re-stat and the POSIX sort run once, after the last part; a failure in any part refuses the folder as a failure to open it does. The walk asks for `BIG_LISTING` entries and enters the semaphore with that first part in hand; the semaphore is now left only once the big listing is let go (under T6's order a second big listing could be read in while the first was resident). **Bounds, asserted at 2, 8 and 64 workers:** at most one listing of more than `BIG_LISTING` entries is resident; the entries resident are at most the largest listing + (W − 1) × `BIG_LISTING`, and the test forces that bound exactly. `WalkCounts` gains `big_resident_peak` and `resident_entries_peak`. A lister that implements only `list` stays unbounded (documented). 57 mutants red; the Linux and Windows listers' own resume tests run on CI's legs.
...
- [ ] **T6c. Per-worker LIFO (work stealing)** (found by T6b, 26 Sep 2026; must land before T12, whose open-set budget needs it): past `q_max` each worker takes from, and pushes to, its own stack of ranges; an idle worker steals the oldest range of another's stack (or the shared FIFO's front). A worker's stack is then one descent, so the ranges queued are at most `q_max + W·D` under every schedule (the classic work-stealing space bound), and the open set is bounded with them.
  - **Tests first:** the exhaustive model's worst shapes (and the real walk on them) stay within `q_max + W·D`; at one worker the listing order is still T6's; T4's lock unchanged. **Mutants:** a shared back stack kept; a thief taking the newest range.
  - **Green gate:** cargo test; tm-walk under blocks-by-default; clippy ×3.
...
3. **Q3 — the 10M row.** §5.3's "10M, full index in memory ≤ 700 MB" cannot be met with P4-1's layout (about 937 MB). Do you accept it met by spill with every row on disk (about 543 MB), with memory mode's threshold at 5M?
   - **Status: decided by the owner on 25 Sep 2026 — the recommendation is taken.** The 10M row is met by spill, with every row kept on disk, and memory mode (`T_mem`) goes up to 5M. The owner's words: "I take the recommendation, met by spill with memory mode up to 5M." ~~**Status: reported to the owner on 24 Sep 2026; the owner's answer is pending.**~~ The reasoning, as reported on 24 Sep 2026: §5.3's "10M entries, full index in memory ≤ 700 MB" is not reachable with P4-1's layout. The walk's merge alone was measured at 147–168 B/node (the runs behind that range are not recorded under `bench/baselines/`), which is 1,470–1,680 MB at 10M by arithmetic. P4-1's store row is 64 B, budget until measured: about 646 MB at 10M with 1% headroom, before the walk's working memory and the transport. ~~Proposed:~~ The row is met by spill, with every row kept on disk (543 MB, budget until measured, §S.3), and memory mode's threshold `T_mem` defaults to 5M. ~~This is a proposal, not a decision.~~ T22 still measures it: the 543 MB is a budget, and the gate is the measured peak.
