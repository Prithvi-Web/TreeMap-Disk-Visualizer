...
## Outcome (24 Sep 2026, ~23:30 UTC, finished by workflow `load-sweep-finish`, run wf_d4e7689d-58e)

* **The two product questions, answered by the owner on 24 Sep 2026 and built:** `GET /api/scan/:id/stats` is in the rate limiter's `meta` lane (`tests/rateLimiterLanes.test.ts`: 100 status polls in one tick all answer and leave the strict lane's 20 untouched; 7 mutants red). A native walk's stall clock (`NATIVE_STALL_MS`) counts only time in which the machine had CPU to spare (`MACHINE_SATURATED_SHARE` 0.9, weighed over `MACHINE_WINDOW_MS` windows, read only while the walk shows nothing new): a walk starved of CPU keeps going, a walk stuck on a dead mount with cores idle still falls back. On Linux the machine is read from `/proc/stat` with iowait counted as idle (`machineTicksFromProcStat`; `os.cpus()` leaves iowait out, so a one-core container with a wedged walk would have read as saturated for ever); a window with no usable reading counts, as before. The sentence names spare CPU only when every counted window was read and had room.
* **Test files:** the four earlier fixer diffs (facts, incrementalRescan, reclaimScoreProvider, safetyRails) were judged sound by skeptics; humanScale (the partial edit finished), savedQueries, notes, polishServerScanRootSymlink, polishServerStats, cartCommit, gduScanner, mcp, sparseFiles, benchScanHold fixed and judged sound; nativeEngine judged weakened once (its standstill check had been dropped), repaired and re-judged sound; nativeEquivalence (c) already robust once the stall rule changed. Every wait on real work is a `waitFor` hang guard; no assertion loosened; each change proven by mutants (controls that stay green are named as controls in the fixers' reports).
* **Found on the way:** `sparseFiles.test.ts` never isolated its app data and saved every scan into the owner's real `~/Library/Application Support/TreeMap` — and it was not alone: 719 of the 2,605 snapshots there were the tests' own temp folders. `scripts/run-tests.js` now gives every `npm test` run its own `TREEMAP_DATA_DIR` (`tests/testDataIsolation.test.ts`); the per-file guard follows in its own commit.
